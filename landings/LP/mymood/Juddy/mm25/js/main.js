const app = new Vue({
    ...
    data: {
        ...
        messages: [
            { text: 'Привет. чем занимаешься? Как тебе фотка?', type: 'message', delay: 2 },
            { text: '', type: 'photo', delay: 4 },
            { text: 'Фото 18+ Подтвердите, чтобы посмотреть', type: 'btnText', delay: 6 },
        ],
        activeMessages: [],
        dialogVisible: false, // Флаг видимости диалогового окна
        currentUser: {}, // Данные текущего выбранного пользователя
        duration: 5, // Таймер в секундах
        maxDuration: 5, // Максимальное время для расчета прогресса
        size: 100, // Размер SVG
    ...
    methods: {
        ...
        showMessages() {
            this.messages.forEach((message) => {
                setTimeout(() => {
                    if (this.ageConfirm) { // Проверяем, чтобы сообщения добавлялись только при ageConfirm = true
        ...
        startTimer() {
            ...
            this.intervalId = setInterval(() => {
                if (this.duration > 0) {
                    ...
                    // Задержка перед миганием кнопки
                    setTimeout(() => {
    ...
    watch: {
        ageConfirm(newValue) {
            if (newValue) {
                this.showMessages(); // Запускаем, только когда ageConfirm становится true
            } else {
                this.activeMessages = []; // Сбрасываем сообщения при переходе назад
            }
        },
    },
    mounted(){
